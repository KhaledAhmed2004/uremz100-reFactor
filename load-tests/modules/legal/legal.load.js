/**
 * legal.load.js — Module entry point for Legal Pages API load testing
 *
 * Usage:
 *   npm run load:legal           → k6 run --out web-dashboard (live dashboard at localhost:5665)
 *   npm run load:legal:stress    → stress scenario only
 *   npm run load:legal:baseline  → baseline scenario only
 *
 * Prerequisites:
 *   1. npm run load:seed:groups  (creates base fixtures)
 *   2. npm run load:seed:legal   (creates legal-specific fixtures)
...
export { runBaseline, runStress, runReadLoad };
...
export const handleSummary = createHandleSummary('legal');
