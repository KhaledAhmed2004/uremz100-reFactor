/**
 * ask-question.load.js — Module entry point for Ask-Question API load testing
 *
 * Usage:
 *   npm run load:ask-question           → k6 run --out web-dashboard (live dashboard at localhost:5665)
 *   npm run load:ask-question:baseline  → k6 run with only baseline scenario
 *   npm run load:ask-question:stress    → k6 run with only stress scenario
 *
 * Prerequisites:
 *   1. npm run load:seed:groups         (creates base fixtures)
 *   2. npm run load:seed:ask-question   (creates ask-question-specific fixtures)
...
export const handleSummary = createHandleSummary('ask-question');
