/**
 * learning-content.load.js — Module entry point for Learning-Content API load testing
 *
 * Usage:
 *   npm run load:learning-content          → k6 run --out web-dashboard (live dashboard at localhost:5665)
 *   npm run load:learning-content:stress   → stress scenario only
 *   npm run load:learning-content:baseline → baseline scenario only
 *
 * Prerequisites:
 *   1. npm run load:seed:groups            (creates base fixtures)
 *   2. npm run load:seed:learning-content  (creates learning-content-specific fixtures)
...
export { runBaseline, runStress, runReadLoad, runWriteLoad, runUserJourney };
...
export const handleSummary = createHandleSummary('learning-content');
