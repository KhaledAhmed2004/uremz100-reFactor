/**
 * mosque.load.js — Module entry point for Mosque API load testing
 *
 * Usage:
 *   npm run load:mosque          → k6 run --out web-dashboard (live dashboard at localhost:5665)
 *   npm run load:mosque:stress   → stress scenario only
 *   npm run load:mosque:baseline → baseline scenario only
 *
 * Prerequisites:
 *   1. npm run load:seed:groups  (creates base fixtures)
 *   2. npm run load:seed:mosque  (creates mosque-specific fixtures)
...
export const handleSummary = createHandleSummary('mosque');
