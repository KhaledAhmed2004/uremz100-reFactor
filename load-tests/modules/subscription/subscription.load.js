/**
 * subscription.load.js — Module entry point for Subscription API load testing
 *
 * Usage:
 *   npm run load:subscription          → k6 run --out web-dashboard (live dashboard at localhost:5665)
 *   npm run load:subscription:stress   → stress scenario only
 *   npm run load:subscription:baseline → baseline scenario only
 *
 * Prerequisites:
 *   1. npm run load:seed:groups        (creates base fixtures)
 *   2. npm run load:seed:subscription  (creates subscription-specific fixtures)
...
export { runBaseline, runStress, runReadLoad, runWriteLoad, runUserJourney, runRateLimit };
...
export const handleSummary = createHandleSummary('subscription');
