/**
 * dua.load.js — Module entry point for Dua API load testing
 *
 * Usage:
 *   npm run load:dua          → k6 run --out web-dashboard (live dashboard at localhost:5665)
 *   npm run load:dua:stress   → stress scenario only
 *   npm run load:dua:baseline → baseline scenario only
 *
 * Prerequisites:
 *   1. npm run load:seed:groups  (creates base fixtures)
 *   2. npm run load:seed:dua     (creates dua-specific fixtures)
...
export const handleSummary = createHandleSummary('dua');
