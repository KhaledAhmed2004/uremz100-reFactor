/**
 * support-ticket.load.js — Module entry point for Support-Ticket API load testing
 *
 * Usage:
 *   npm run load:support-ticket           → k6 run --out web-dashboard (live dashboard at localhost:5665)
 *   npm run load:support-ticket:stress    → stress scenario only
 *   npm run load:support-ticket:baseline  → baseline scenario only
 *
 * Prerequisites:
 *   1. npm run load:seed:groups           (creates base fixtures)
 *   2. npm run load:seed:support-ticket   (creates support-ticket-specific fixtures)
...
export const handleSummary = createHandleSummary('support-ticket');
