...
// Mock fixture data matching the structure of auth-fixtures / base-fixtures
const fixtures = {
  adminUser: {
...
// Import the auth entry point options for scenario configuration validation
// We inline the options here since the entry point uses k6-specific imports that won't work in Node
const authOptions = {
  scenarios: {
    baseline: {
 ...
 * Selection distributes via modulo without producing out-of-bounds indices.
 */
    ...
    fc.assert(
      fc.property(
        ...
        (vuIndex, role) => {
          ...
          // Verify modulo index is within bounds
          expect(expectedIndex).toBeGreaterThanOrEqual(0);
          expect(expectedIndex).toBeLessThan(pool.length);

          // Verify the returned user matches the expected pool entry
 ...
 * **Validates: Requirements 8.1, 8.2, 8.3, 8.4, 8.5**
 *
 * For the auth entry point scenario configuration, every scenario has:
...
describe('Property 2: Scenario Configuration Validity', () => {
  const scenarioNames = Object.keys(authOptions.scenarios);
  ...
  it('all scenarios have valid executor types, positive VU counts, and non-empty exec names', () => {
    fc.assert(
      fc.property(
        ...
        (scenarioName) => {
          const scenario = authOptions.scenarios[scenarioName];

          // Must have a valid executor type
          expect(VALID_EXECUTORS).toContain(scenario.executor);

          // Must have positive VU count
