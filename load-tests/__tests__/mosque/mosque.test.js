...
// Inline mosque scenario options (k6-specific imports won't work in Node)
const mosqueOptions = {
  scenarios: {
    baseline: {
 ...
 * **Validates: Requirements 14.1, 14.2, 14.3, 14.4, 14.5**
 *
 * For the mosque entry point scenario configuration, every scenario has:
...
describe('Property 2: Scenario Configuration Validity', () => {
  const scenarioNames = Object.keys(mosqueOptions.scenarios);
  ...
  it('all scenarios have valid executor types, positive VU counts, and non-empty exec names', () => {
    fc.assert(
      fc.property(
        ...
        (scenarioName) => {
          const scenario = mosqueOptions.scenarios[scenarioName];
