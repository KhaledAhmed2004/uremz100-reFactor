...
const getMovieAnalyticsOverviewData = async (id: string) => {
  ...
  const totalViewsLast48Hours = viewersData.filter(v => {
    const watchedAt = new Date(v.lastWatchedAt || (v as any).createdAt);
...
const getEpisodeAnalyticsOverviewData = async (id: string) => {
...
const getEpisodeAnalyticsEngagementData = async (id: string) => {
  const episode = await Episode.findById(id);
  if (!episode) return null;
...
const getEpisodeAnalyticsAudienceData = async (id: string) => {
  const episode = await Episode.findById(id);
  if (!episode) return null;
